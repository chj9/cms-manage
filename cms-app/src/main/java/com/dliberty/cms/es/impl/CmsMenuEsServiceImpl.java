...
public class CmsMenuEsServiceImpl implements CmsMenuEsService {
    ...
    public Page<CmsMenuVo> pageQuery(Integer pageNo, Integer pageSize, String kw) {
        ...
        if (StringUtils.isEmpty(kw)) {
            NativeSearchQuery query = new NativeSearchQueryBuilder().withQuery(matchAllQuery()).withPageable(page).withSorts(SortBuilders.fieldSort("browseNum").order(SortOrder.DESC)).build();
            //return cmsMenuRepository.search(query);
            return null;
        } else {
            // 多字段查找
            MultiMatchQueryBuilder builder = new MultiMatchQueryBuilder(kw).field("menuName", 10).field("menuDesc", 5).field("categoryName", 5).field("stepList.stepDesc", 9).field("materialList.materialName", 10).field("labelList.labelName", 9).fuzziness(Fuzziness.AUTO);

            NativeSearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(builder).withPageable(page)
    ...
    public Page<CmsMenuVo> pageQueryByCateId(Integer pageNo, Integer pageSize, Integer categoryId) {
        ...
        NativeSearchQuery searchQuery = new NativeSearchQueryBuilder().withQuery(tqb).withPageable(PageRequest.of(pageNo, pageSize)).withSorts(SortBuilders.fieldSort("browseNum").order(SortOrder.DESC)).build();
        //return cmsMenuRepository.search(searchQuery);
        return null;
    }
