...
import com.chj9.cms.elasticsearch.service.EsService;
import com.chj9.cms.elasticsearch.vo.IndexDataToSave;
...
public class CmsMenuEsServiceImpl implements CmsMenuEsService {
    ...
    public List<CmsMenuVo> getByIds(List<String> ids) {
        try {
            List<CmsMenuVo> list = esService.searchByIn(WRITE_ALIAS_NAME, "id", ids, CmsMenuVo.class);
            return list;
        } catch (Exception e) {
            LOGGER.message("查询失败")
                    .context("ids",ids)
